...
function setupDays(){
	var dates = document.getElementById("delivDy").getElementsByTagName("Optional");
}
...
function setupDays(){
	...
	twentyNine.appendChild(dates[28].cloneNode(true)); //adds 29th
	
	thirty.appendChild(dates[28].cloneNode(true));
	thirty.appendChild(dates[29].cloneNode(true)); //adds 29th and 30th
	
	thirtyOne.appendChild(dates[28].cloneNode(true));
	thirtyOne.appendChild(dates[29].cloneNode(true));
	thirtyOne.appendChild(dates[30].cloneNode(true)); //adds 29th, 30th and 31st
}
...
function updateDays(){
	...
	while (dates[28]) {
		//remove  child with index of 28 until this index is emptyBoxes
...
function generatePlaceholder() {
	if (!Modernizr.input.placeholder) {
		var messageBox = document.getElementById("customText");
		messageBox.value = messageBox.placeholder;
		messageBox.style.color = "rgb(179,184,183)";
...
function autocheckCustom() {
	...
	if(messageBox.vavlue !== "" && messageBox.value !== messageBox.placeholder) {
...
function validateAddress(fieldsetId) {
	...
	try {
		...
		if (fieldsetValidity === false) {
			// throw approriate message based on current fieldsetId
...
function validatePayment() {
	...
	try {
		...
		if (cvvElement.value === "") {
			//verify that the cvv value has been entered
...
function validateMessage() {
	...
	try {
		if (document.getElementById("custom").checked && ((msgBox.value === "") || (msgBox.value === msgBox.placeholder))) {
			//custom checked but message box emptyBoxes
...
/*validate number fields for older browswers*/
...
/* run initial form configuation functions */
function setUpPage() {
